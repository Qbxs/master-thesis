...
This permits many useful features in programming languages such as the reuse and abstraction of code to the supertypes and implicit coercions from a subtype to a supertype.
  ...
  \begin{prooftree}
    ...
    \UnaryInfC{$\tau \meet \rho \sub \sigma$}
  \end{prooftree}

  \begin{prooftree}
    \AxiomC{$\tau \sub \sigma$}
    \RightLabel{\textsc{Meet}$_2$}
    \UnaryInfC{$\rho \meet \tau \sub \sigma$}
  \end{prooftree}
...
\begin{quote}
    The intuitive idea of a eubtype is one whose
objects provide all the behavior of objects of another type (th e supertype) plus something extra. What is
wanted here is something like the following substitution property: If for each object $\sigma_1$ of type $S$ there
is an object $\sigma_2$ of type $T$ such that for all programs $P$ defined in terms of $T$, the behavior of $P$ is
unchanged when $\sigma_1$ is substituted for $\sigma_2$, then $S$ is a subtype of $T$.
\end{quote}
...
E.g. the function \mintinline{Haskell}{chooseSecond :: a -> Int -> Int} is a subtype of \mintinline{Haskell}{const :: a -> b -> b}.

Similarily, the hierarchy of type classes expresses the same relationship exhibited by subtyping.
% example?
