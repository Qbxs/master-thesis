...
More interesting, \todo{reformulate} perhaps, is the usage of parametric data structures such as lists.
...
% ???
In the end, this enables us to use elaborate concepts such as functors and monads to reason about programs. \todo{questionable standalone phrase}
...
So instead of a type class method \todo{maybe use $\Phi_C$ instead of $C$}$f : \forall \alpha. C \alpha \Rightarrow \tau[\alpha]$ and instance $C \alpha_1, \dots, C \alpha_n$,
...
Here, overlapping or otherwise ambiguous instances are simply not allowed to be declared in the same namespace. \todo{more specific}
