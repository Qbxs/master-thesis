...
Generally, union types and the $\bot$ type can only occur in output positions, while intersection types and the $\top$ type can only occur in inout positions. \cite{dolan2017subtyping}
Therefore, we introduce \emph{polar types} as a means to distinguish positive types (for outputs) and negative types (for inputs).
...
\begin{figure}
  \begin{flalign*}
    \tau^+ := & \; \Nat \; | \; \Int \; | \; \Bool \; | \; \Unit \; | \; \String \; | \; \Singleton{0} \; | \; \Singleton{1} \; | \; \Singleton{2} \; | \dots \\
