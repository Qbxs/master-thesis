...
If for each object $\sigma_1$ of type $S$ there is an object $\sigma_2$ of type $T$ such that for all programs $P$ defined in terms of $T$,
the behavior of $P$ is unchanged when $\sigma_1$ is substituted for $\sigma_2$, then $S$ is a subtype of $T$.
\end{quote}
...
Subtyping allows us to pass such more specific arguments in a more natural way and makes the coercion implicit.
