...
Otherwise, we could have the following two derivations with different witnesses if there is a primitive subtyping derivation for $\tau \sub \sigma$:
...
\begin{center}
  ...
  \DisplayProof
  {\hskip.2in}
  \AxiomC{$j : {\bf instance} \; \Phi^+(\sigma)$}
  \RightLabel{\textsc{Decl}}
  \UnaryInfC{$w' : \Phi^+(\sigma)$}
  \RightLabel{\textsc{Prim}}
  \UnaryInfC{$coerce(w') : \Phi^+(\tau)$}
  \DisplayProof
\end{center}

For contravariant predicates and $\sigma \sub \tau$ we could have:
