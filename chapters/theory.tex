...
Additionally, we introduce a trivial rule for axioms:
...
$\Phi(\tau) \Rightarrow \Psi(\tau)$ means $\Psi$ is constrained by $\Psi$.
Thus, if $\Psi(\tau)$ is satisfied, then the constraint $\Phi(\tau)$.
