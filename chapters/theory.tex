...
We obtain the following derivations for $\Showable(\mathcal{U} \meet \mathbb{N})$ and $\Defaultable(\mathbb{Z} \join \sigma)$:
...
\begin{prooftree}
  ...
  \alwaysSingleLine
  \RightLabel{\textsc{Join}$^-_1$}
  \UnaryInfC{$\ctx \Defaultable(\mathbb{Z} \join \sigma)$}
\end{prooftree}
...
We again differentiate co- and contravariant type classes in order to transport type classes over the subtyping relation.
 ...
 \begin{center}
  \AxiomC{}
  \RightLabel{\textsc{Top}}
  \UnaryInfC{$\mathit{fromTop} : \tau \sub \top$}
  \DisplayProof
  {\hskip.2in}
  \AxiomC{}
  \RightLabel{\textsc{Bot}}
  \UnaryInfC{$\mathit{toBot} : \bot \sub \tau$}
  \DisplayProof
\end{center}
