\chapter{Theory of Type Classes} \label{ch:theory}
...
Since all witnesses of $\tau$ are also witnesses of $\sigma$, the property $\Phi$ is propagated to $\tau$.
  ...
  These are also \emph{admissable} since for any $\tau$ and $\sigma$ we have with the subtyping rules $\tau < \tau \join \sigma$ and $\sigma < \tau \join \sigma$ and can then derive this rule from \textsc{Sub}.
