...
\begin{figure}[h]
  ...
  \begin{flalign*}
  & \instance{\Showable}{\mathbb{N}} : \\
  & \; \; \mathit{show} : \mathbb{N} \to \mathit{String} \\
...
\caption{Incoherent instance declarations for $\Showable(\mathbb{N})$}
\end{figure}
...
If we call the method $\mathit{show}$ for an argument of type $\mathbb{N}$,
...
\begin{prooftree}
  ...
  \UnaryInfC{$\refl_\tau : \tau \sub \tau$}
  \RightLabel{\textsc{CoV}}
  \BinaryInfC{$\cov(i_/tau(w),\refl_\tau) : \Phi^+(\tau)$}
\end{prooftree}
