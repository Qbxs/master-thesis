...
We might motivate this rule because $\Phi^<$ already satisfies both components of the union type.
However, rule is not derivable and therefore not sound, so instead we have to use the more restricted version above.
...
The \textsc{Meet}$^>$ rule can not be derived with the subtyping rules either because $\tau \meet \sigma$ is neither a supertype of $\tau$ nor of $\sigma$.
...
However, generally a term of type $\tau$ or of type $\sigma$ is not of type $\tau \meet \sigma$.
% this rule is bogus:
...
Therefore, these non-derivable rules cannot be used for type class resolution and should be rejected.
